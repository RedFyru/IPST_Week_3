...
import { checkSharePermission, checkToDoOwner } from "../../common/middleware/check-owner";
...
export const todoRouter = async (app: FastifyInstance) => {
    ...
    app.get("/:id", { schema: getToDoByIdSchema, preHandler: app.auth([checkToDoOwner, checkSharePermission]) }, toDoController.getToDoById);
    app.post("/:id/share", { schema: shareToDoSchema, preHandler: app.auth([checkToDoOwner]) }, toDoController.shareToDo);
